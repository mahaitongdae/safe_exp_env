...
class EnvironmentModel(object):  # all tensors
    def __init__(self, task, num_future_data=0, rewards_mode='penalty'):
        self.task = task
        self.mode = None
    ...
    def reset(self, obses, ref_indexes=None):  # input are all tensors
        ...
        self.actions = None
        self.reward_info = None
        self.realveh2vehAlast = 2.5 * tf.ones([256, 32])
    ...
    def rollout_out(self, actions):  # obses and actions are tensors, think of actions are in range [-1, 1]
        with tf.name_scope('model_step') as scope:
            ...
            if self.rewards_mode == 'barrier':
                rewards, veh2veh4training, veh2road4training, veh2veh4real, veh2road4real = self.compute_rewards(self.obses, self.actions)
        ...
        # return self.obses, rewards, punish_term_for_training, veh2veh4training, veh2veh4real, veh2road4real
    ...
    def barrier_lambda_schedule(self, ite):
        ...
        if ite <= end_ite:
            factor =  tf.cast(ite//interval, tf.float32) / tf.cast(self.args.max_updated_steps//interval, tf.float32)
            self.barrier_lambda = (init_lambda - end_lambda) *(1 - factor)  + end_lambda
        else:
            self.barrier_lambda = tf.convert_to_tensor(end_lambda)
    ...
    def compute_rewards(self, obses, actions):
        ...
        with tf.name_scope('compute_reward') as scope:
            ...
            realveh2vehAlast = tf.expand_dims(tf.zeros_like(veh_infos[:, 0]),1)
            if self.realveh2vehAlast is None:
                self.realveh2vehAlast = 2.5 * tf.ones_like(veh_infos)
            ...
            for veh_index in range(int(tf.shape(veh_infos)[1] / self.per_veh_info_dim)):
                ...
                for ego_point in [ego_front_points, ego_rear_points]:
                    for veh_point in [veh_front_points, veh_rear_points]:
                        ...
                        if not self.rewards_mode == 'barrier':
                            realveh2vehAlast = None
                            veh2veh4training += tf.where(veh2veh_dist - 3.5 < 0, tf.square(veh2veh_dist - 3.5),
                                                         tf.zeros_like(veh_infos[:, 0]))
                            # veh2veh4real += tf.where(veh2veh_dist - 2.5 < 0, veh2veh_dist - 2.5,
                            #                          tf.zeros_like(veh_infos[:, 0]))
            ...
            if self.rewards_mode == 'single_projection':
                punish_term_for_training = 0 * veh2veh4training + veh2road4training
                real_punish_term = 0 * veh2veh4real + veh2road4real
                ...
                return rewards, punish_term_for_training, veh2veh4training, veh2veh4real, veh2road4real
            if self.rewards_mode == 'penalty':
            ...
            if self.rewards_mode == 'barrier':
                # punish_term_for_training = veh2veh4training + veh2road4training
                real_punish_term = veh2veh4real + veh2road4real
                return rewards, veh2veh4training, veh2road4training, veh2veh4real, veh2road4real
